if(Function.prototype.apply && Element.prototype.dispatchEvent){
	...
	var ASML = function(){
		...
		asml.viewPort = (function(){
			...
			oWrapper.addEventListener('scroll', function(){
				console.log("scroll")
				self.offset({
					l: oWrapper.pageXOffset,
					r: -oWrapper.pageXOffset,
					b: -oWrapper.pageYOffset,
					t: oWrapper.pageYOffset,
				});
			}, false)
		...
		var XML = {
			...
			node: function(prmA, prmB, prmC){
				switch(prmA.constructor){
					case Array:
						var element
						var ns = prmB
						switch(prmA[0].constructor){
							case String:
								element = document.createElementNS(ns, prmA[0])
								break
							default:
								element = prmA[0]
								if(ns === undefined) ns = element.namespaceURI
							if(ns === undefined) ns = XML.NS.HTML
						}
						if(prmA[1] !== undefined){
							switch(prmA[1].constructor){
								case Object:
									var attr = prmA[1]
									for(a in attr){
							...
							if(prmA[2] !== undefined){
								switch(prmA[2].constructor){
									case String:
										element.innerHTML = prmA[2]
										break
									case Array:
										prmA[2].forEach(function(node){
											element.appendChild( XML.node(node, ns) )
										})
										break
								}
							}
						...
						return element
					case String:
						return document.createTextNode(prmA)
					default:
						// Must be a DOM Element
						return XML.node([prmA, prmB, prmC])
				}
			},
		}
