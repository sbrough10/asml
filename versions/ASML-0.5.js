var ASML = function(){
	var ListNode = function(prev, value, next){
		this.prev = prev;
		this.value = value;
		this.next = next;
		this.cutTo = function(node){
			this.next = node;
			node.prev = this;
		}
	};

	var dispatchNewEvent = function(node, type, data){
			...
			if(isDef(prm)){
				if(!attrChange.changing){
					bufferedAction = function(){
						...
						self.runEvent("change"+changeName, {
							effectees: attrChange.effectees.slice(0, attrChange.effectees.length),
						});
